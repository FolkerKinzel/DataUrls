...
public static class DataUrlExample
{
    public static void Example()
    {
        ...
        if (info.TryGetData(out OneOf<string, byte[]> data))
        {
            data.Switch(
               static str => Console.WriteLine("Embedded string: {0}", str),
               static bytes => Console.WriteLine("Data Length:    {0} Bytes", bytes.Length)
               );
        }
    }
    ...
    private static string CreatePhotoFile()
    {
        ...
        if (DataUrl.TryGetData(url, out OneOf<string, byte[]> data, out string? fileTypeExtension)
